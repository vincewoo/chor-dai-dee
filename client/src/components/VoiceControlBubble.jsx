...
const VoiceControlBubble = ({
  ...
  // ⚡ Bolt Optimization: Consume audio levels from volatile context to avoid parent re-renders
  const { audioLevels } = useVoiceAudio();

  // Prefer context level if available (and username is known)
  const audioLevel = (username && audioLevels && audioLevels[username] !== undefined)
    ? audioLevels[username]
    : propAudioLevel;
