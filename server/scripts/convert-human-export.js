...
function convert(records, { subject, gamma }) {
    ...
    return { rows, eligible: eligible.length, skipped, trajectories: byRoundSeat.size };
}

function main(argv = process.argv) {
    const args = parseArgs(argv);
    if (args.help) { console.log(USAGE); return 0; }
    if (!args.input || !args.subject) {
        throw new Error('--input and --subject are required');
    }
    ...
    if (!result.rows.length) throw new Error('No eligible human decisions were converted');
